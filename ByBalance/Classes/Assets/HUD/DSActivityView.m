...
static BOOL isShowAnimate = NO;
static BOOL isShowBackground = YES;
...
+ (DSActivityView *)newActivityView
{
    return [self newActivityViewForView:[[UIApplication sharedApplication] keyWindow] withLabel:nil width:0];
}
...
{
	isShowAnimate = isAnimated;
}

+ (void) showBackground:(BOOL) isBackground
{
	isShowBackground = isBackground;
}
...
- (DSActivityView *)initForViewAsInstance:(UIView *)addToView withLabel:(NSString *)labelText width:(NSUInteger)labelWidth;
{
	if (!(self = [super initWithFrame:CGRectZero]))
		return nil;
...
- (UILabel *)activityLabel;
{
    if (!_activityLabel)
    {
        ...
        _activityLabel.shadowOffset = CGSizeMake(0.f, 1.f);
    }
...
- (void)layoutSubviews;
{
    ...
    borderFrame.size.width = self.activityIndicator.frame.size.width + textSize.width + 25.f;
    borderFrame.size.height = self.activityIndicator.frame.size.height + 10.f;
    borderFrame.origin.x = floorf(0.5f * (self.frame.size.width - borderFrame.size.width));
    borderFrame.origin.y = floorf(0.5f * (self.frame.size.height - borderFrame.size.height - 20.f));
	...
	indicatorFrame.origin.x = 10.f;
	indicatorFrame.origin.y = 0.5f * (borderFrame.size.height - indicatorFrame.size.height);
    ...
    labelFrame.origin.x = borderFrame.size.width - labelFrame.size.width - 10.f;
	labelFrame.origin.y = floorf(0.5f * (borderFrame.size.height - labelFrame.size.height));
...
- (void)setupBackground;
{
	...
	{
		self.backgroundColor = [[UIColor blackColor] colorWithAlphaComponent:0.35f];
	}
}
...
- (UIView *)borderView;
{
    if (!_borderView)
    {
        ...
        _borderView.backgroundColor = [[UIColor blackColor] colorWithAlphaComponent:0.5f];
        _borderView.layer.cornerRadius = 10.f;
    }
...
- (void)layoutSubviews;
{
    ...
    if (textSize.width < self.activityIndicator.frame.size.width)
        textSize.width = self.activityIndicator.frame.size.width + 10.f;
	...
	CGRect borderFrame = CGRectZero;
    borderFrame.size.width = textSize.width + 30.f;
    borderFrame.size.height = self.activityIndicator.frame.size.height + textSize.height + 40.f;
    borderFrame.origin.x = floorf(0.5f * (self.frame.size.width - borderFrame.size.width));
    borderFrame.origin.y = floorf(0.5f * (self.frame.size.height - borderFrame.size.height));
    self.borderView.frame = borderFrame;
	
    // Calculate the position of the indicator: horizontally centered and near the top of the border view:
    CGRect indicatorFrame = self.activityIndicator.frame;
	indicatorFrame.origin.x = 0.5f * (borderFrame.size.width - indicatorFrame.size.width);
	indicatorFrame.origin.y = 20.f;
    self.activityIndicator.frame = indicatorFrame;
    
    // Calculate the position of the label: horizontally centered and near the bottom of the border view:
	CGRect labelFrame = self.activityLabel.frame;
    labelFrame.origin.x = floorf(0.5f * (borderFrame.size.width - labelFrame.size.width));
	labelFrame.origin.y = borderFrame.size.height - labelFrame.size.height - 10.f;
    self.activityLabel.frame = labelFrame;
}
...
- (void)animateShow;
{
	...
	{
		self.alpha = 0.f;
		self.borderView.transform = CGAffineTransformMakeScale(3.f, 3.f);
		
		[UIView beginAnimations:nil context:nil];
		//	[UIView setAnimationDuration:5.f];            // Uncomment to see the animation in slow motion
...
- (void)animateRemove;
{
    ...
    self.borderView.transform = CGAffineTransformMakeScale(0.5f, 0.5f);
    self.alpha = 0.f;
...
- (UIView *)keyboardView;
{
	...
	for (UIWindow *window in [windows reverseObjectEnumerator])
	{
		...
		{
            // UIPeripheralHostView is used from iOS 4.f, UIKeyboard was used in previous versions:
